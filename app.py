...
df = pd.read_csv('https://raw.githubusercontent.com/DuaneIndustries/roaster-rebuild-dash/main/STACK%20BUDGET3.csv')
...
df['Budget Number'] = df['Budget Number'].str.replace(",","").astype(float)
df['Cost to Date'] = df['Cost to Date'].replace("","0")
df['Cost to Date'] = df['Cost to Date'].str.replace(",","").astype(float)
    ...
    style_cell_conditional=[
            {
                'if': {'column_id': c},
                'textAlign': 'left'
            } for c in ['index_col', 'Project Section','Labor / Materials', 'item','Status']
        ],
        style_data={
            'whiteSpace': 'normal',
            'height': 'auto'}
...
def update_gantt(all_rows_data, slctd_row_indices, slct_rows_names, slctd_rows,
        ...
        return [
            dcc.Graph(id="gantt-chart",
             figure = px.timeline(
                ...
                hover_name='item',
                hover_data=['Budget Number']).update_yaxes(autorange='reversed').update_layout(
    hovermode="closest",
    title_x=0.45,
    ...
    if labor and total not in dff['Labor / Materials'].values:
        dfb = dff.sort_values(["Progress"], ascending=True)
        return [
        ...
        Ttl = df[df['Labor / Materials']=='Total']
        totalbudget = Ttl['Budget Number'].sum().astype(int)
        totalcost = Ttl['Cost to Date'].sum().astype(int)
        return[
            dcc.Graph(id="pie-chart",
                      figure=px.pie(
                          ...
                          hover_data=['Cost to Date'],
                          opacity=.75
                      ).update_layout(
                          title=f'Overall Project Cost : ${totalcost} of ${totalbudget}',
                          title_x=0.50,
                          title_font_size=24,
                          paper_bgcolor='dimgray',
                          plot_bgcolor='dimgray',
